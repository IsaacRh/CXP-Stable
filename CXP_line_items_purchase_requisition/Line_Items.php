...
function display_requisition_lines($focus, $field, $value, $view)
{
    ...
    $html = '';
    $html .= '<link rel="stylesheet" href="modules/CXP_line_items_purchase_requisition/css/style.css">';

    if ($view == 'EditView') {
        $html .= '<script src="modules/CXP_line_items_purchase_requisition/js/line_items.js"></script>'; 
        ...
        if ($focus->id != '') {
            ...
            $sql = "SELECT pg.id FROM CXP_line_items_purchase_requisition pg  WHERE pg.cxp_purchase_requisition_id_c='".$focus->id."' AND pg.deleted = 0 ";
            ...
            while ($row = $focus->db->fetchByAssoc($result)) {
                $line_item = BeanFactory::newBean('CXP_line_items_purchase_requisition');
        ...
        while ($row = $focus->db->fetchByAssoc($result)) {
            $line_item = new CXP_line_items_purchase_requisition();
            ...
            if ($line_item->id != '0') {
                ...
                $product .= "<td class='tabDetailViewDF' style='padding:2px;'>".stripDecimalPointsAndTrailingZeroes2(format_number($line_item->quantity), $sep[1])."</td>";
...
function stripDecimalPointsAndTrailingZeroes2($inputString, $decimalSeparator)
{
    return preg_replace('/'.preg_quote($decimalSeparator).'[0]+$/', '', $inputString);
}

function get_discount_string2($type, $amount, $params, $locale, $sep)
{
...
function display_shipping_vat2($focus, $field, $value, $view)
{
    if ($view == 'EditView') {
