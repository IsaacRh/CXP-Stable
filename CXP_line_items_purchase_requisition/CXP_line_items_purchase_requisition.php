...
require_once('modules/CXP_line_items_purchase_requisition/CXP_line_items_purchase_requisition_sugar.php');
class CXP_line_items_purchase_requisition extends CXP_line_items_purchase_requisition_sugar
{
        ...
        for ($i=0; $i <= $line_count ; $i++) { 
            if (isset($data['product_name'][$i])) {
                if (isset($data['product_deleted'][$i]) && $data['product_deleted'][$i] == 1) {
                    ...
                    $product = BeanFactory::getBean('CXP_line_items_purchase_requisition', $postDataKeyIdI);
                    if (!$product) {
                        $product = BeanFactory::newBean('CXP_line_items_purchase_requisition');
                    }
                    ...
                    if (trim($product->name) != '') {
                        ...
                        $product->cxp_purchase_requisition_id_c = $parent->id;
                        ...
                        $key = Relationship::retrieve_by_modules('CXP_line_items_purchase_requisition', 'CXP_unit_measure', $GLOBALS['db']);
                        if (!empty($key)) {
