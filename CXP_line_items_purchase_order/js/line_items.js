 ...
 */
 $(document).ready(() => {
    ...
    document.querySelector("[data-label='LBL_CXP_PURCHASE_ORDER_CXP_SUPPLIERS_FROM_CXP_SUPPLIERS_TITLE']").innerHTML = "\nProveedores: ";
    $("#discount, #withholdings, #iva").blur(() => { calculateTotal(); });
    calculateTotal();
  });
  ...
  */
  ...
  function insertGroup(){
     ...
     footer_cell.innerHTML = "<input type='button' tabindex='116' class='button add_product_line' value='Agregar Producto Insumo' id='"+productTable.id+"addProductLine' onclick='insertProductLine(\""+productTable.id+"\",\""+groupn+"\", \""+ input + "\")' />";
     footer_cell.innerHTML += "<input type='button' tabindex='116' class='button add_product_line' value='Agregar Producto Materia Prima' id='"+productTable.id+"addProductLine' onclick='insertProductLine(\""+productTable.id+"\",\""+groupn+"\", \""+ raw + "\")' />";
     groupn++;
     return groupn -1;
  }
  ...
  function calculateTotal(key){
    ...
    for (let i=0; i < length; i++) {
      ...
      for (let j=0; j < input.length; j++) {
          if (input[j].id.indexOf('product_quantity') != -1) {
          qty = unformat2Number(input[j].value);
          }
          if (input[j].id.indexOf('product_price') != -1)
          {
            price = unformat2Number(input[j].value);
          }
      ...
      if(deleted !== 1){
        tot_amt += eval(price * qty);
      }
    }
  
    var withholdings = get_value('withholdings');
    if(isNaN(withholdings)){ withholdings = 0; set_value("withholdings", 0); }
  
    var iva = get_value("iva");
    if(isNaN(iva)) { iva = 0; set_value("iva", 0); }
    
    var discount = get_value("discount");
    if(isNaN(discount)) { discount = 0; set_value("discount", 0); }
   
    subtotal = eval(tot_amt - discount);
    var total_amount = eval(subtotal - withholdings + iva);
  
    set_value("subtotal_one", tot_amt);
    set_value("subtotal_two", subtotal);
    set_value("neto_total", total_amount);
